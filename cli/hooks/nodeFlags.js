...
function resolveHeapFlags(env = process.env) {
  const explicit = String(env.TOKENPROXY_MAX_OLD_SPACE_SIZE ?? "").trim();
  if (explicit) {
    ...
    console.warn(
      `[tokenproxy] ignoring TOKENPROXY_MAX_OLD_SPACE_SIZE="${explicit}": expected a positive integer (MB) or 0`,
    );
  }
