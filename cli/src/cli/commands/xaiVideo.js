/**
 * `tokenproxy xai video` — generate a Grok Imagine video through the local
 * tokenproxy gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: tokenproxy xai video --prompt "..." [options]

Generate a Grok Imagine video via your local tokenproxy gateway
...
Options:
  ...
  --api-key <key>         tokenproxy API key (or env TOKENPROXY_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.TOKENPROXY_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-tokenproxy-connection-id"] || null;
