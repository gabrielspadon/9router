...
async function buildCodexHeader() {
  ...
  const { installed, hasTokenProxy, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasTokenProxy, settings } = result.data;
  ...
  // Extract TokenProxy custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:TokenProxy-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract TokenProxy provider config
  const provider = settings?.models?.providers?.["tokenproxy"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("tokenproxy/") ? primary.replace("tokenproxy/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasTokenProxy, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use TokenProxy\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
