...
sync() {
  ...
  mkdir -p "$DATA_DIR/db"
  sqlite3 "$HOME/.tokenproxy/db/data.sqlite" ".backup '$DATA_DIR/db/data.sqlite'"
  rm -f "$DATA_DIR"/db/data.sqlite-wal "$DATA_DIR"/db/data.sqlite-shm
  for f in usage.json log.txt; do
    [ -f "$HOME/.tokenproxy/$f" ] && cp "$HOME/.tokenproxy/$f" "$DATA_DIR/$f" 2>/dev/null || true
  done
