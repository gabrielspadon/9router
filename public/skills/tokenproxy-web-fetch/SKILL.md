---
name: tokenproxy-web-fetch
description: Fetch URL → markdown / text / HTML via TokenProxy /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# TokenProxy — Web Fetch
...
```bash
curl $TOKENPROXY_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$TOKENPROXY_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $TOKENPROXY_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $TOKENPROXY_URL/v1/web/fetch \
...
```js
const r = await fetch(`${process.env.TOKENPROXY_URL}/v1/web/fetch`, {
  method: "POST",
