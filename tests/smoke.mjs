...
//
// What it validates, against a spawned tokenproxy server:
...
async function main() {
  console.log(`tokenproxy free-model smoke — mode=${MODE} live=${LIVE} port=${PORT}`);
  ...
  const dataDir = mkdtempSync(join(tmpdir(), "tp-smoke-"));
