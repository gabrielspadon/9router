...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("tokenproxy");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "tokenproxy-general-purpose",
      explore: "tokenproxy-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.tokenproxy\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.tokenproxy-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.tokenproxy-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# tokenproxy-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.tokenproxy-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("tokenproxy-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.tokenproxy-");
    expect(reset).not.toContain("tokenproxy-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("tokenproxy-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("tokenproxy-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("tokenproxy-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("tokenproxy-explore");
    expect(getGrokSubagentSlot("plan")).toBe("tokenproxy-plan");
