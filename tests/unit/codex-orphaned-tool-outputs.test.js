...
describe("Codex orphaned tool outputs", () => {
  it("restores the typed placeholder after function-only orphan cleanup", () => {
    expect(transform([
      { type: "function_call_output", call_id: "gone-function", output: "secret" },
    ])).toEqual([
      { type: "message", role: "user", content: [{ type: "input_text", text: "..." }] },
    ]);
  });

  it("restores the typed placeholder after custom-only orphan cleanup", () => {
    expect(transform([
      { type: "custom_tool_call_output", call_id: "gone-custom", output: "secret" },
    ])).toEqual([
      { type: "message", role: "user", content: [{ type: "input_text", text: "..." }] },
    ]);
  });
