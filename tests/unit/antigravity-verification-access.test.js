...
beforeEach(() => {
  ...
  mocks.hasTrustedPeerHeaders.mockImplementation((request) => request.headers.get("x-9r-peer-token") === "trusted");
});
...
describe("Antigravity verification access", () => {
  describe.each([true, false])("requireLogin=%s", (requireLogin) => {
    it.each(identityRows)("authorizes $name only through approved identity", async (row) => {
      ...
      const headers = {
        ...(row.trusted ? { "x-9r-peer-token": "trusted" } : {}),
        ...(row.realIp ? { "x-9r-real-ip": row.realIp } : {}),
        ...(row.proxied ? { "x-9r-via-proxy": "1" } : {}),
      };
