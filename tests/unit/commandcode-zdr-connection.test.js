...
async function setup() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "tokenproxy-commandcode-zdr-"));
...
function createRequest() {
  return new Request("http://tokenproxy.local/api/providers", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
function updateRequest(zdrEnabled) {
  return new Request("http://tokenproxy.local/api/providers/id", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
