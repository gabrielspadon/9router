...
describe("XaiExecutor suffix parsing", () => {
  ...
  test("keeps none reasoning_effort for grok-4", () => {
    const body = { model: "grok-4", reasoning_effort: "none", messages: [] };
    const out = exec.transformRequest("xai/grok-4", body);
    expect(out.reasoning_effort).toBe("none");
  });

  test("strips none reasoning_effort for Grok Composer", () => {
    const body = { model: "grok-composer-2.5-fast", reasoning_effort: "none", messages: [] };
    const out = exec.transformRequest("xai/grok-composer-2.5-fast", body);
    expect(out.reasoning_effort).toBeUndefined();
  });
});
