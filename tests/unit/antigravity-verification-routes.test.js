...
beforeEach(() => {
  ...
  mocks.toProxyOptions.mockImplementation((proxyConfig) => proxyConfig);
});
...
describe("Antigravity verification routes", () => {
  ...
  describe("recheck", () => {
    ...
    it("forwards a selected strict route into the recheck probe", async () => {
      const strictOptions = {
      ...
      mocks.resolveProxy.mockResolvedValue({ kind: "usable", strictProxy: true });
      mocks.toProxyOptions.mockReturnValue(strictOptions);
      mocks.usable.mockReturnValue(true);
      mocks.getVerification.mockReturnValueOnce(current).mockReturnValue(null);
      ...
      expect(mocks.runProbe).toHaveBeenCalledWith(
        connection,
        strictOptions,
        { force: true, expectedChallengeId: current.challengeId },
      );
    });

    it("returns required-proxy unavailable before a recheck probe", async () => {
      mocks.resolveProxy.mockResolvedValue({ kind: "required-unavailable" });
