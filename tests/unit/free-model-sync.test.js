...
const CATALOGS = {
  ...
  // #1995: kilocode is category "oauth" but shares the "openrouter-free"
  // filter contract, so its shape mirrors the openrouter fixture above.
  "api.kilo.ai/api/gateway/models": () =>
    jsonResponse({
      data: [
        { id: "kilo-auto/free", name: "Auto Free", pricing: { prompt: "0", completion: "0" }, context_length: 256000 },
        { id: "paid/kilo-model", pricing: { prompt: "0.001", completion: "0.002" }, context_length: 300000 }, // paid → filtered
      ],
    }),
};
...
describe("getSyncTargets", () => {
  ...
  it("includes an oauth-category provider only when its own fetcher is a *-free contract (#1995)", async () => {
    // kilocode is category "oauth" but modelsFetcher.type is "openrouter-free",
    // a contract that already narrows to zero-priced models, so it qualifies.
    const { getSyncTargets } = await import("../../src/shared/services/freeModelSync.js");
    const ids = getSyncTargets().map((t) => t.id);
...
describe("runFreeModelSync", () => {
  it("persists filtered catalogs, diffs customs and rewrites auto-combos", async () => {
    ...
    // #1995: kilocode now reaches the sync loop and gets its own free-only snapshot.
    expect(state.freeModels.kilocode.ids).toEqual(["kilo-auto/free"]);
    ...
    // #1995: kilocode's free model reaches customModels under its uiAlias "kc",
    // which is what ModelSelectModal.js reads for a passthroughModels provider.
    expect(state.customModels.get("kc|kilo-auto/free|llm")).toBeTruthy();
