...
describe("convertResponsesStreamToJson usage passthrough", () => {
  ...
  it("keeps exact provider cost for internal persistence without serializing it", async () => {
    const out = await convertResponsesStreamToJson(
      sseFromEvents(baseEvents({
        input_tokens: 300,
        output_tokens: 120,
    ...
    expect(out.usage.cost_usd).toBe(0.25);
    expect(out.usage.cost_in_usd).toBe(0.2);
    expect(out.usage.cost_in_usd_ticks).toBe(2_500_000_000);
    expect(JSON.stringify(out)).not.toContain("cost_usd");
  });
