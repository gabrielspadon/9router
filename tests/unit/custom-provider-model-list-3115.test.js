...
vi.mock("@/lib/network/connectionProxy", () => ({
  ...
  isRequiredProxyUnavailableError: vi.fn(() => false),
}));
...
describe("compatible-provider public model allowlists", () => {
  ...
  it.each([
    [401, "unauthorized", "provider_catalog_unauthorized", false, "Update the connection credentials, then retry the import."],
    [404, "not_found", "provider_catalog_not_found", false, "Check the connection endpoint or add models manually."],
    [429, "rate_limited", "provider_catalog_rate_limited", true, "Wait for the provider rate limit to reset, then retry the import."],
  ])("maps upstream status %i to an actionable %s catalog failure", async (status, state, code, retryable, action) => {
    const log = vi.spyOn(console, "log").mockImplementation(() => {});
    fetchMock.mockResolvedValueOnce({ ok: false, status, text: async () => "upstream details" });
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      catalog: { state, code, retryable, action },
    });
    expect(log.mock.calls).not.toContainEqual([
      expect.stringMatching(/^Error fetching models from/),
      "upstream details",
    ]);
    log.mockRestore();
  });
  ...
  it.each([
    ...
    Date.parse("2020-01-01T00:00:00.000Z") / 1000,
  ])("distinguishes expired credential value %s from a generic authorization failure", async (expiresAt) => {
    providerConnectionMock.mockResolvedValueOnce({
      ...connection,
      expiresAt,
    });
    ...
    await expect(response.json()).resolves.toMatchObject({
      catalog: {
        ...
        code: "provider_catalog_credential_expired",
        retryable: false,
  ...
  it("reports a provider without catalog support as a manual-import state", async () => {
    providerConnectionMock.mockResolvedValueOnce({
      ...connection,
      provider: "provider-without-a-catalog",
    });
    ...
    await expect(response.json()).resolves.toMatchObject({
      catalog: {
        state: "unsupported",
        code: "provider_catalog_unsupported",
        retryable: false,
  ...
  it("reports a provider catalog timeout separately from other transport failures", async () => {
    fetchMock.mockRejectedValueOnce(Object.assign(new Error("timed out"), { code: "ETIMEDOUT" }));
