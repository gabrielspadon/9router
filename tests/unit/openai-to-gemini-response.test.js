...
describe.each([FORMATS.GEMINI, FORMATS.GEMINI_CLI, FORMATS.VERTEX])(
  ...
  (clientFormat) => {
    it('projects OpenAI chunks into the Gemini response.candidates envelope', () => {
      ...
      const out = openaiChunks.flatMap(
        (chunk) => translateResponse(FORMATS.OPENAI, clientFormat, chunk, state) || []
      );

      expect(out[0].response.candidates[0].content.parts[0]).toEqual({
        thought: true,
        text: 'think',
      });
