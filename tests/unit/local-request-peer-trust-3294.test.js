...
// path, so x-tp-real-ip arrives straight from the client and a remote caller can claim to
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-tp-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-tp-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting tokenproxy with a reverse proxy", () => {
