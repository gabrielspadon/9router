 ...
 * the proxy environment entirely; only tokenproxy applied it.
 *
...
async function withStubbedFetch(env) {
  ...
  globalThis.fetch = async (url, options) => {
    calls.push({
      url: String(url),
      hasDispatcher: options?.dispatcher !== undefined,
      // The invariant is that loopback never goes THROUGH the proxy, which is
      // narrower than "no dispatcher at all": the direct path legitimately
      // carries a plain undici Agent for Happy Eyeballs. A ProxyAgent here is
      // the bug #3424 was about; an Agent is not.
      dispatcherKind: options?.dispatcher?.constructor?.name ?? null,
    });
...
describe("proxyAwareFetch honours the bypass (#3424)", () => {
  ...
  it("does not attach a proxy dispatcher for a loopback target when HTTP_PROXY is set", async () => {
    ...
    expect(harness.calls[0].dispatcherKind).not.toBe("ProxyAgent");
  });
  ...
  it("still routes a NON-loopback target through the proxy, so the bypass is not a blanket opt-out", async () => {
    harness = await withStubbedFetch({ HTTP_PROXY: "http://corp-proxy:3128", NO_PROXY: "" });

    await harness.proxyAwareFetch("http://ollama.internal.example:11434/api/chat", {
      method: "POST",
    });

    expect(harness.calls).toHaveLength(1);
    expect(harness.calls[0].dispatcherKind).toBe("ProxyAgent");
  });
});
