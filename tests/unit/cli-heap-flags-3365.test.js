...
describe("resolveHeapFlags (#3365)", () => {
  ...
  it("honours an explicit cap", () => {
    expect(resolveHeapFlags({ TOKENPROXY_MAX_OLD_SPACE_SIZE: "384" }))
      .toEqual(["--max-old-space-size=384"]);
    expect(resolveHeapFlags({ TOKENPROXY_MAX_OLD_SPACE_SIZE: " 1024 " }))
      .toEqual(["--max-old-space-size=1024"]);
  });

  it("emits no flag at all for 0, leaving the sizing to node", () => {
    expect(resolveHeapFlags({ TOKENPROXY_MAX_OLD_SPACE_SIZE: "0" })).toEqual([]);
  });
  ...
  it("prefers the dedicated var over NODE_OPTIONS", () => {
    const flags = resolveHeapFlags({
      TOKENPROXY_MAX_OLD_SPACE_SIZE: "512",
  ...
  it("falls back to the default on a junk value, and says so", () => {
    for (const value of ["abc", "-1", "1.5", "512MB"]) {
      expect(resolveHeapFlags({ TOKENPROXY_MAX_OLD_SPACE_SIZE: value }), value).toEqual([DEFAULT_FLAG]);
    }
    expect(warn).toHaveBeenCalledTimes(4);
    expect(warn.mock.calls[0][0]).toContain("TOKENPROXY_MAX_OLD_SPACE_SIZE");
  });

  it("treats an empty or whitespace value as unset, without warning", () => {
    expect(resolveHeapFlags({ TOKENPROXY_MAX_OLD_SPACE_SIZE: "   " })).toEqual([DEFAULT_FLAG]);
