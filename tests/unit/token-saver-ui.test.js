...
const OBS = SRC.slice(
  SRC.indexOf('aria-label="Token Saver aggregate statistics"'),
);
...
describe("token-saver stats polling (30s) and Headroom state branches", () => {
  ...
  it("declares module constants REFRESH_MS = 30_000 and TIMEOUT_MS = 10_000", () => {
    expect(SRC).toMatch(/const\s+TOKEN_SAVER_STATS_REFRESH_MS\s*=\s*30_000/);
    // hanging fetch must not starve polling forever
    expect(SRC).toMatch(/const\s+TOKEN_SAVER_STATS_TIMEOUT_MS\s*=\s*10_000/);
  });
