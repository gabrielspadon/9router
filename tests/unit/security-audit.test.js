...
describe("AUDIT-002: API key masking", () => {
  ...
  it("getUsageStats should expose masked display fields with opaque identities", () => {
    const source = fs.readFileSync(
      fromRepo("src/lib/db/repos/usageRepo.js"),
      "utf-8"
    );
    // Both code paths (daily summary + 24h live) retain the masked display.
  ...
  it("byApiKey object keys should use an opaque identity, never a raw key", () => {
    const source = fs.readFileSync(
      fromRepo("src/lib/db/repos/usageRepo.js"),
      "utf-8"
    );
    expect(source).toContain("aggregateKey: `${apiKeyKey}|${rawModel}|${rawProvider}`");
    expect(source).toContain("id:${keyInfo.id}");
    expect(source).toContain("hmac:${createHmac");
...
describe("AUDIT-003: Proxy URL validation", () => {
  beforeEach(() => {
    ...
    delete process.env.TOKENPROXY_PROXY_MANAGED;
    delete process.env.TOKENPROXY_PROXY_URL;
    delete process.env.TOKENPROXY_NO_PROXY;
