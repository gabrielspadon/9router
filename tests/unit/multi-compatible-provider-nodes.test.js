...
async function setup() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "tokenproxy-multi-provider-"));
...
function makeRequest(overrides = {}) {
  return new Request("https://tokenproxy.local/api/provider-nodes", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
describe("multi-protocol compatible provider nodes", () => {
  ...
  it("updates endpoint transports on existing connections", async () => {
    ...
    await ctx.createConnection(new Request("https://tokenproxy.local/api/providers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ provider: node.id, apiKey: "test-key", name: "Connection" }),
    }));

    const updateResponse = await ctx.PUT(new Request(`https://tokenproxy.local/api/provider-nodes/${node.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: "Multi API",
  ...
  it("validates each configured protocol endpoint", async () => {
    ...
    const response = await ctx.validate(new Request("https://tokenproxy.local/api/provider-nodes/validate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        type: "multi-compatible",
