...
async function setupDb() {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "tokenproxy-model-routing-"),
  );
...
describe("model routing", () => {
  ...
  it("routes to custom node when its prefix collides with a built-in provider alias", async () => {
    ...
    await expect(
      ctx.getModelInfo("cf/@cf/black-forest-labs/flux-2-klein-9b"),
    ).resolves.toEqual({
  ...
  it("routes multi-protocol node prefixes", async () => {
    ...
    await expect(ctx.getModelInfo("multi/shared-model")).resolves.toEqual({
  ...
  it("routes to custom node when prefix matches built-in tokenrouter alias (tr)", async () => {
    ...
    await expect(ctx.getModelInfo("tr/qwen/qwen3.8-max-free")).resolves.toEqual(
      {
