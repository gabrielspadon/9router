...
- [ ] Baseline regression gate reports no regression
  ...
  cd tests && npx vitest run --testTimeout=30000 --hookTimeout=30000 \
    --reporter=default --reporter=json --outputFile.json=/tmp/tokenproxy-vitest.json
  cd .. && node tests/__baseline__/verify-no-regression.mjs /tmp/tokenproxy-vitest.json
  ```
