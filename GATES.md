...
## fork-safety
- [ ] CHECK: `test "$(git remote get-url --push upstream)" = DISABLED_NO_PUSH_TO_UPSTREAM && git ls-remote https://github.com/gabrielspadon/9router.git refs/heads/master | cut -f1` EXPECT: final fork master SHA recorded
  EVIDENCE: pending

## pull-request-backlog
- [ ] CHECK: `node scripts/tracking/sync-upstream.mjs --check && test "$(rg -c '^## PR #' tracking/upstream-prs-open.md || true)" = 0` EXPECT: tracking state OK and zero reachable open PR entries
  EVIDENCE: pending

## issue-backlog
- [ ] CHECK: `node scripts/tracking/sync-upstream.mjs --check && test "$(rg -c '^## Issue #' tracking/upstream-issues-open.md || true)" = 0` EXPECT: tracking state OK and zero reachable open issue entries
  EVIDENCE: pending

## repository-professionalization
- [ ] Documentation, governance, security, CI, dependency, release, license, support, and contribution surfaces are audited and verified against their authoritative commands and hosted state.
  EVIDENCE: pending

## visual-polish
- [ ] Desktop and responsive dashboard workflows, accessibility, console, network, and production build behavior have current saved browser evidence after final visual polish.
  EVIDENCE: pending

## final-verification
- [ ] The complete no-regression suite, registry and alias baselines, build, production smoke path, tracking duplicate checks, clean-tree check, and fork remote equality all pass on the final tree.
  EVIDENCE: pending
