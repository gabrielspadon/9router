...
3. Rank eligible accounts by expiring usable entitlement across all applicable windows. A sooner reset receives more urgency, while a constraining longer window prevents a short window from overspending it. Use deterministic account priority only as a tie-breaker. Rank each account on the evidence that account reports: accounts of one provider routinely report different window sets, because plan tier and recent usage both change which windows a provider bothers to report, so a differing window shape must never be a reason to stop ranking the pool. An account whose evidence is missing or unreadable ranks last and stays in service, since an evidence gap is not evidence of depletion. A recorded reset that has already elapsed describes a window that has replenished, and it must be projected forward onto the current period before it is used as an ordering key, or the account that was just handed a fresh period sorts ahead of every account whose entitlement is genuinely about to expire.
4. Pin a client session to the selected account. Do not round-robin between healthy accounts. Keep the pin until the account becomes unavailable, the operator drains it, or a model-specific failure requires repin. A healthy pin is not surrendered to an account that has just reset, and not to one that merely ranks higher. A switch abandons the pinned account's prompt-cache prefix, so the next request re-primes the whole conversation at full input price: the operator pays cash for every switch, and the only switch worth paying for is one that buys entitlement that would otherwise be lost.
5. Depletion is the decision point. When the pinned account exhausts a binding window, choose again from the whole eligible pool, which folds back in any account that has restocked while the session was living elsewhere. Return to a restocked account when its own projected deadline lands before the next candidate's, and not otherwise: earliest means earliest DEADLINE, because the deadline is what decides whose tokens get wasted. It does not mean earliest in an operator-declared account order, which rule 3 already demotes to a tie-break. Never spray one session across all accounts to increase instantaneous throughput. When every account reads depleted, hold the session on its own account and let the upstream answer rather than refusing on a snapshot that may be hours old; surface a rate limit to the caller only once no account in the pool can serve, and carry the earliest real reset with it rather than a retry floor.
