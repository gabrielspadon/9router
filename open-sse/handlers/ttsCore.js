 ...
 */
export async function handleTtsCore({ provider, model, input, credentials, responseFormat = "mp3", language, style, providerOptions = {} }) {
  if (!input?.trim()) {
  ...
  try {
    ...
    if (adapter) {
      const result = await adapter.synthesize(input.trim(), model, credentials, responseFormat, { language, style, providerOptions });
