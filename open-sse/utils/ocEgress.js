...
// (client-supplied x-real-ip values are stripped by the CDN in front of
// opencode.ai). Every IP has a daily request budget (reset at UTC midnight).
// When the current egress IP budget is exhausted the gateway answers
// 429 FreeUsageLimitError and it stays exhausted until midnight — the only
// way to "get more quota" is to send the next requests from a DIFFERENT real
// egress IP. The user picks the egress manually (proxy node in the proxy UI,
// or flipOcEgress()); the executor never switches on its own.
//
...
// Mode is persisted in <APPDATA|HOME>/tokenproxy/oc-egress.json so it survives
...
function egressFile() {
  ...
  return path.join(base, 'tokenproxy', EGRESS_FILE);
}
