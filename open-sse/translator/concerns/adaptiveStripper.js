/**
 * Adaptive unsupported-parameter stripper. When an upstream provider rejects a
 * request field (400 "property X is unsupported"), remember the rejection per
 * provider+model and strip the field from subsequent requests automatically,
 * including an immediate one-shot retry of the failed request.
 *
 * Adapted from an upstream pull request in the predecessor project.
 */
...
export function extractRejectedFieldNamesFromError(responseBody) {
  if (typeof responseBody === 'string') {
    return extractRejectedFieldNames(responseBody);
  }
  if (typeof responseBody === 'object' && responseBody !== null) {
    ...
    if (responseBody.error?.message) {
      return extractRejectedFieldNames(responseBody.error.message);
    }
  }
  return [];
}
