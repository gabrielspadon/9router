...
// every local tokenproxy user into one shared "127.0.0.1"/LAN bucket that
...
export class OpenCodeExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials, _sourceFormat, targetFormat) {
    ...
    // The Responses-to-Chat translator can resolve an OpenAI Chat Completions
    // payload without its original stream flag. OpenCode's Chat endpoint needs
    // both stream=true and include_usage to return the final usage chunk.
    const resolvedWireFormat = targetFormat ?? this.config.format;
    if (
      resolvedWireFormat === "openai" &&
      stream === true &&
      Array.isArray(transformed?.messages)
    ) {
      transformed.stream = true;
      if (transformed.stream_options?.include_usage === undefined) {
  ...
  // picks another node/egress themselves and the SAME conversation continues
  // from the new IP. applyOcEgress() only honors the manually chosen mode
  // from the state file (<APPDATA|HOME>/tokenproxy/oc-egress.json) so a manual
  // switch works without restarting tokenproxy. Errors propagate untouched.
  async execute(args) {
    applyOcEgress();
  ...
  buildHeaders(credentials, stream = true) {
    ...
    // header; no header = ONE global bucket shared with every other
    ...
    // x-tp-real-ip, which is 127.0.0.1 for local clients — forwarding that
    // would put every local tokenproxy user into one shared loopback bucket.
    // For loopback/private peers we fall back to the machine's own public IP.
    const rawIp = (lower["x-tp-real-ip"] || lower["x-real-ip"] || "").trim();
