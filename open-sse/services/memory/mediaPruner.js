...
export function pruneHistoricalMedia(body, options = {}) {
  ...
  for (let i = 0; i <= lastAssistantIndex; i++) {
    ...
    if (Array.isArray(msg.images) && msg.images.length > 0) {
      ...
      msg.images = [];
      msg.content = `${typeof msg.content === "string" ? msg.content : ""}\n[Historical images removed by tokenproxy memory optimizer]`.trim();
    }
    ...
    if (typeof msg.content === "string") {
      if (msg.content.includes("data:image/") || msg.content.includes("data:audio/")) {
        const replaced = msg.content.replace(/data:(image|audio|video)\/[a-zA-Z0-9.+_-]+;base64,[A-Za-z0-9+/=]+/g, "[Historical base64 media omitted by tokenproxy]");
    ...
    if (Array.isArray(msg.content)) {
      ...
      for (const block of msg.content) {
        if (isMediaBlock(block)) {
          ...
          newContent.push({
            type: "text",
            text: `[Historical ${mediaType} omitted by tokenproxy memory optimizer]`,
          });
        } else {
    ...
    if (Array.isArray(msg.parts)) {
      ...
      for (const part of msg.parts) {
        if (part?.inlineData || part?.fileData) {
          savedItems++;
          newParts.push({
            text: `[Historical media omitted by tokenproxy memory optimizer]`,
          });
        } else {
          newParts.push(part);
        }
