...
const ALLOWED_DOMAINS = new Set([
  ...
  // The chat transport runs on the daily host (ANTIGRAVITY_IDE_BASE_URL), so a
  // VALIDATION_REQUIRED 403 raised during inference carries this domain. Without
  // it here the source:"chat" path could never classify, and an account Google
  // has gated behind browser verification surfaced only as an opaque failure
  // that re-auth cannot clear (#2632).
