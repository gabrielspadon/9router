    ...
    <div className="space-y-5.5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <h2 className="text-lg font-semibold text-text-main flex items-center gap-2">
          Memory & Context
          <Tooltip text="Bounds historical tool and media content, compacts older context, and carries handoffs between sessions." />
        </h2>
        <a
          href="https://github.com/akitaonrails/ai-memory"
          target="_blank"
          rel="noreferrer"
          className="focus-ring hit-44 inline-flex items-center gap-1 rounded-[4px] text-xs text-text-muted underline hover:text-brand"
        >
          ai-memory
      ...
      <Card id="tool-pruning">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-main">Historical Tool Output Pruning</h3>
          </div>
          <Toggle checked={memoryToolPruningEnabled} onChange={handleMemoryToolPruning} ariaLabel="Historical tool output pruning" />
        </div>
        <p className="text-sm text-text-muted mb-4">
          Bounds older file reads, build logs, and git diffs while keeping the
          configured recent tool turns intact.
        </p>
        ...
        {memoryToolPruningEnabled && (
          <div className="bg-surface-2 p-4 rounded-[var(--radius-brand)] border border-border grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <label htmlFor="memory-tool-turns-keep-full" className="block text-xs font-medium text-text-muted mb-1">
                Keep Recent Tool Turns Full
              </label>
              <input
                id="memory-tool-turns-keep-full"
                type="number"
                min="1"
                max="10"
                ...
                className="focus-ring min-h-11 w-full rounded-[var(--radius-brand)] border border-border bg-surface px-3 py-1.5 font-mono text-sm text-text-main metric"
              />
            ...
            <div>
              <label htmlFor="memory-max-historical-tool-chars" className="block text-xs font-medium text-text-muted mb-1">
                Max Historical Output Length (chars)
              </label>
              <input
                id="memory-max-historical-tool-chars"
                type="number"
                min="100"
                max="5000"
                step="100"
      ...
      <Card id="media-pruning">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-main">Historical Media & Attachment Pruning</h3>
          </div>
          <Toggle checked={memoryMediaPruningEnabled} onChange={handleMemoryMediaPruning} ariaLabel="Historical media and attachment pruning" />
        </div>
        <p className="text-sm text-text-muted">
          Replaces heavy Base64 image and audio payloads in older answered turns with lightweight references (`[Historical media omitted by tokenproxy]`), preserving full media exclusively in the current active user turn.
        </p>
      </Card>
      ...
      <Card id="compaction">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-main">Sliding Window Context Compaction</h3>
          </div>
          <Toggle checked={memoryCompactionEnabled} onChange={handleMemoryCompaction} ariaLabel="Sliding window context compaction" />
        </div>
        <p className="text-sm text-text-muted mb-4">
        ...
        {memoryCompactionEnabled && (
          <div className="bg-surface-2 p-4 rounded-[var(--radius-brand)] border border-border grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <label htmlFor="memory-compaction-threshold-tokens" className="block text-xs font-medium text-text-muted mb-1">
                Compaction Token Threshold
              </label>
              <input
                id="memory-compaction-threshold-tokens"
                type="number"
                min="4000"
                max="128000"
                step="4000"
            ...
            <div>
              <label htmlFor="memory-recent-turns-to-keep" className="block text-xs font-medium text-text-muted mb-1">
                Recent Turns to Keep Intact
              </label>
              <input
                id="memory-recent-turns-to-keep"
                type="number"
                min="2"
                max="30"
      ...
      <Card id="handoff">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-main">Cross-Session Handoff Continuity</h3>
          </div>
          <Toggle checked={memoryHandoffEnabled} onChange={handleMemoryHandoff} ariaLabel="Cross-session handoff continuity" />
        </div>
        <p className="text-sm text-text-muted">
