    ...
    <div className="space-y-5.5">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          <a href="/dashboard/token-saver" className="focus-ring hit-44 rounded-sm text-xs text-brand underline hover:no-underline">
          ...
          <div className="flex items-center gap-1 rounded-[var(--radius-brand)] border border-border bg-surface-2 p-1">
            {WINDOW_TABS.map((tab) => (
              <button
                ...
                className={`focus-ring hit-44 px-3 py-1 rounded-[var(--radius-brand)] text-xs font-medium transition-colors duration-150 ${
                  windowId === tab.id
        ...
        {stats?.timeline?.some((d) => d.tokensSavedEst > 0) ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={stats.timeline} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11 }} tickFormatter={fmtTokens} width="auto" />
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              {/* Size/duration columns keep physical `text-right`: numeric
            ...
            <tbody>
              {(stats?.recent || []).slice(0, 50).map((ev, i) => (
                <tr key={`${ev.ts}-${i}`} className="border-b border-border-subtle">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-[var(--radius-brand)] border ${
                        ev.applied
              ...
              {(!stats?.recent || stats.recent.length === 0) && (
                <tr>
                  <td colSpan={8} className="px-4 py-5.5 text-center text-text-muted text-sm">
