...
import { Card, Button, ConfirmModal, Input, Toggle } from "@/shared/components";
import {
  CLAUDE_ROLE_KEYS,
  emptyClaudeDefaults,
...
// Which keys a write changes, against what the file holds right now.
function describeEnvWrite(nextEnv, currentEnv) {
  const current = currentEnv && typeof currentEnv === "object" && !Array.isArray(currentEnv) ? currentEnv : {};
  const overwrites = [];
  const additions = [];
  const unchanged = [];
  for (const [key, value] of Object.entries(nextEnv || {})) {
    if (!Object.prototype.hasOwnProperty.call(current, key)) additions.push({ key, to: value });
    else if (current[key] === value) unchanged.push(key);
    else overwrites.push({ key, from: current[key], to: value });
  }
  return { overwrites, additions, unchanged };
}

// What the operator reads before this destructive write runs. It overwrites
// the live Claude Code configuration of the machine running TokenProxy, so the
// confirmation names the absolute file and every key whose value changes;
// "some env keys" is not good enough to click through.
export function claudeWriteConfirmation({ file, env, currentEnv }) {
  const { overwrites, additions, unchanged } = describeEnvWrite(env, currentEnv);
  const keyCount = Object.keys(env || {}).length;
  const lines = [
    {
      text: file
        ? `This overwrites ${file}, the live Claude Code configuration on the machine running TokenProxy.`
        : "This overwrites ~/.claude/settings.json (the server did not report the resolved path),"
          + " the live Claude Code configuration on the machine running TokenProxy.",
    },
  ];
  if (!currentEnv) {
    lines.push({
      text: `Its current contents could not be read, so any of the ${keyCount} key(s) below may already hold a different value.`,
      tone: "warning",
    });
  }
  if (overwrites.length) {
    lines.push({ text: `Replaces ${overwrites.length} existing key(s):`, strong: true });
    for (const { key, from, to } of overwrites) {
      lines.push({ text: `${key}: ${String(from)} → ${String(to)}`, mono: true });
    }
  }
  if (additions.length) {
    lines.push({ text: `Adds ${additions.length} new key(s):`, strong: true });
    for (const { key, to } of additions) {
      lines.push({ text: `${key}: ${String(to)}`, mono: true });
    }
  }
  if (unchanged.length) {
    lines.push({ text: `${unchanged.length} key(s) already hold this value and stay as they are.` });
  }
  lines.push({
    text: "Nothing else in the file changes. Every other env key and every non-env setting is preserved.",
  });
  return lines;
}
...
export default function ClaudeCompatPage() {
  ...
  // Pending destructive write, awaiting confirmation.
  ...
  // Stage the (possibly hand-edited) env JSON for confirmation. The write lands
  // on the real ~/.claude/settings.json of the machine running the gateway, so
  // the current file is read first and the exact path plus every changing key
  // are put in front of the operator before anything is overwritten.
  const handleWriteClaudeSettings = async () => {
    ...
    let currentEnv = null;
    try {
      const res = await fetch("/api/cli-tools/claude-settings", { cache: "no-store" });
      const data = await res.json();
      file = data?.settingsPath || null;
      const env0 = data?.settings?.env;
      currentEnv = env0 && typeof env0 === "object" && !Array.isArray(env0) ? env0 : null;
    } catch {
      // Left null — the confirmation says the current file could not be read.
    }
    setPendingWrite({ env, lines: claudeWriteConfirmation({ file, env, currentEnv }) });
  };

  // Server applies the payload as a strict key-overwrite on the env object of
  // ~/.claude/settings.json.
  const confirmWriteClaudeSettings = async () => {
    const env = pendingWrite?.env;
    if (!env) return;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <Card id="claude-compat">
        <div className="flex items-center justify-between mb-4">
          ...
          <Toggle
            ...
            ariaLabel="Let Anthropic-protocol clients discover models"
          />
        </div>
        ...
        <div className="flex items-center justify-between py-3 border-t border-border">
          ...
          <select
            ...
            className="focus-ring min-h-11 text-sm text-text-main border border-border rounded-[var(--radius-brand)] px-2 py-1.5 bg-surface-2"
          >
        ...
        {claudeCompatEnabled && (
          <div className="mt-3 pt-3 border-t border-border">
            ...
            <div className="grid grid-cols-[minmax(80px,max-content)_1fr_1fr_44px] gap-2 text-xs text-text-muted pb-1">
              ...
              <span>Model</span>
              {/* Physical text-right: heads a numeric toggle column. */}
            ...
            {CLAUDE_ROLE_KEYS.map((role) => (
              <div key={role} className="grid grid-cols-[minmax(80px,max-content)_1fr_1fr_44px] gap-2 items-center py-1">
                ...
                />
                {/* The label is the target: a 16px checkbox is under the 32px
                    floor in design-system.md section 6, and the grid column is
                    already 44 wide, so the activation region takes the column. */}
                <label className="flex h-11 cursor-pointer items-center justify-end">
                  <input
                    type="checkbox"
                    checked={claudeDefaults[role].oneM}
                    onChange={(e) => updateClaudeRole(role, "oneM", e.target.checked)}
                    className="focus-ring w-4 h-4 accent-brand-500"
            ...
            <textarea
              ...
              className="focus-ring w-full rounded-[var(--radius-brand)] border border-border bg-surface-2 p-3 font-mono text-xs text-text-main leading-relaxed"
            />
            {claudeWriteMsg && (
      ...
      {/* The write overwrites the operator's live Claude Code configuration.
          Name the file and every changing key before it happens. */}
      <ConfirmModal
        isOpen={!!pendingWrite}
        onClose={() => setPendingWrite(null)}
        onConfirm={confirmWriteClaudeSettings}
        title="Overwrite Claude settings?"
        confirmText="Overwrite settings.json"
        variant="danger"
        loading={claudeWriteBusy}
        message={pendingWrite && (
          <>
            {pendingWrite.lines.map((line, i) => (
              <span
                key={i}
                className={`block ${line.mono ? "font-mono text-xs break-all" : "mb-2"} ${
                  line.tone === "warning" ? "text-warning" : line.strong ? "font-medium text-text-main" : ""
                }`}
              >
                {line.text}
              </span>
...
function ClaudeModelField({ value, onChange, options }) {
  ...
  return (
    <select
      ...
      className="focus-ring min-h-11 text-sm text-text-main border border-border rounded-[var(--radius-brand)] px-2 py-1.5 bg-surface-2 w-full"
    >
