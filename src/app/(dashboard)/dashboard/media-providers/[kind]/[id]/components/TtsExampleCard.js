      ...
      <Card>
        ...
        <div className="flex flex-col gap-3">
          ...
          {config.hasBrowseButton && (
            <Row label="Language">
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                <button
                  onClick={openModal}
                  className="focus-ring w-full min-w-0 flex-1 px-3 py-1.5 text-sm border border-border rounded-[var(--radius-brand)] bg-surface-2 font-mono truncate text-start hover:border-brand-line transition-colors duration-150"
                >
                  {selectedLang
                ...
                <button
                  onClick={openModal}
                  className="focus-ring flex w-full items-center justify-center gap-1 text-xs px-3 py-1.5 rounded-[var(--radius-brand)] border border-border text-text-muted hover:text-brand hover:border-brand-line transition-colors duration-150 sm:w-auto sm:shrink-0"
                >
              ...
              <div className="flex flex-wrap gap-1.5">
                {countryVoices.map((v) => (
                  <button
                    ...
                    className={`focus-ring px-3 py-1 rounded-full text-xs border transition-colors duration-150 ${
                      selectedVoice === v.id
                  ...
                  >
                    ...
                    {v.free_users_allowed === true && (
                      <span className="ms-1.5 px-1 py-1 font-mono text-[10.5px] font-semibold rounded bg-success-soft text-success border border-success-line">Free</span>
                    )}
                    {v.free_users_allowed === false && (
                      <span className="ms-1.5 px-1 py-1 font-mono text-[10.5px] font-semibold rounded bg-warning-soft text-warning border border-warning-line">Paid</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-1.5 pe-8 text-sm border border-border rounded-[var(--radius-brand)] bg-surface-2 text-text-main focus-ring font-mono"
                  />
                  {voiceId && (
                    <Button
            ...
            <Row label={translate("Style")}>
              <div className="relative">
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-1.5 pe-8 text-sm border border-border rounded-[var(--radius-brand)] bg-surface-2 text-text-main focus-ring resize-none"
                />
                {style && (
                  <Button
                    variant="bare" size="icon-sm"
          ...
          {audioUrl ? (
            <div>
              ...
              {jsonResponse && (
                <div className="mt-3">
                  ...
                  <pre className="bg-surface-2 rounded-[var(--radius-brand)] px-3 py-3 text-xs font-mono text-text-main overflow-x-auto whitespace-pre-wrap break-all">
                    {JSON.stringify({
            ...
            <pre className="mt-1.5 bg-surface-2 rounded-[var(--radius-brand)] px-3 py-3 text-xs font-mono text-text-main overflow-x-auto whitespace-pre-wrap break-all opacity-50">{DEFAULT_TTS_RESPONSE_EXAMPLE}</pre>
          </div>
          )}
        </div>
      </Card>
                ...
                <div className="flex flex-col gap-1">
                  {filteredLanguages.map((c) => (
                    <button
                      ...
                      className={`focus-ring flex items-center justify-between w-full px-3 py-2 rounded-[var(--radius-brand)] text-start hover:bg-surface-2 transition-colors duration-150 ${
