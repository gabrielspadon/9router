  ...
  return (
    <div className="flex flex-col gap-5.5">
      ...
      <div>
        ...
        >
          <span className="material-symbols-outlined dir-icon text-[18px]" aria-hidden="true">arrow_back</span>
          {kindConfig.label}
        </Link>
      ...
      {!isCustom && provider.kindNotice?.[kind] && (
        <div className="flex items-start gap-3 p-4 rounded-[var(--radius-brand)] bg-warning-soft border border-warning-line text-warning">
          <span className="material-symbols-outlined text-[20px] mt-1 shrink-0" aria-hidden="true">warning</span>
        ...
        <div className="flex flex-col gap-2 rounded-[var(--radius-brand)] border border-info-line bg-info-soft p-4 sm:flex-row sm:items-center">
          ...
          {provider.notice.apiKeyUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="focus-ring inline-flex shrink-0 justify-center rounded-[var(--radius-brand)] bg-info-solid px-2 py-1 text-xs font-medium text-info-on transition-colors duration-150 hover:bg-info-solid-hover sm:py-1"
            >
              Get API Key →
            </a>
          )}
