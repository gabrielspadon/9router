      ...
      <Card padding="xs" className={`h-full hover:bg-surface-2 transition-colors cursor-pointer ${allDisabled ? "opacity-50" : ""}`}>
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">{renderStatus()}</div>
          </div>
        </div>
      </Card>
    </Link>
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-1 sm:shrink-0">
                ...
                {combo.models.length > 6 && (
                  <span className="font-mono text-[10.5px] text-text-muted ms-1 metric">+{combo.models.length - 6}</span>
                )}
              </div>
              <span className="text-xs text-text-muted shrink-0 metric">{combo.models.length}</span>
              <span className="material-symbols-outlined dir-icon text-text-muted text-[16px]" aria-hidden="true">chevron_right</span>
            </div>
          </Card>
        </Link>
      ))}
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="material-symbols-outlined text-[20px] text-text-muted" aria-hidden="true">{icon}</span>
          <h2 className="text-sm font-semibold text-text-main">{title}</h2>
          <span className="text-xs text-text-muted metric">({providers.length} providers · {combos.length} combos)</span>
        </div>
      ...
      {providers.length === 0 ? (
        <div className="text-center py-8 border border-dashed border-border rounded-[var(--radius-brand-lg)] text-text-muted text-sm">
          No providers.
        </div>
      ) : (
