...
function ModelContextPage() {
  ...
  // Below `sm` the filter controls stacked one per line and pushed the table off
  // the first screen, so there they collapse behind a toggle. The toggle carries
  // the count of conditions actually narrowing the list, because `statusF` starts
  // at "active" and a filter already on behind a closed panel is a trap.
  ...
  // What the phone toggle has to report while the controls are out of sight.
  const activeFilterCount =
    (q ? 1 : 0) + (providersSel.length ? 1 : 0) + (statusF === "all" ? 0 : 1) +
    (covF === "all" ? 0 : 1) + (connF === "all" ? 0 : 1) + (minW ? 1 : 0) + (maxW ? 1 : 0);
  ...
  return (
    <div className="flex flex-col gap-5.5">
      ...
      <Card padding="none" className="min-w-0">
        {/* Toolbar. Above `sm` this is the single wrapped row it has always been:
            the filter group carries `sm:contents`, so its controls sit directly
            in this flex row as before. Below `sm` they are a disclosure instead,
            opened by the toggle that precedes them in the DOM as on screen. */}
        <div className="flex flex-col gap-3 px-4 py-3 border-b border-border-subtle sm:flex-row sm:flex-wrap sm:items-center">
          <Button
            size="sm"
            variant={activeFilterCount > 0 ? "secondary" : "ghost"}
            icon={filtersOpen ? "expand_less" : "expand_more"}
            aria-expanded={filtersOpen}
            aria-controls="mc-filters"
            onClick={() => setFiltersOpen(!filtersOpen)}
            className="self-start sm:hidden"
          >
            Filter <span className="metric">{activeFilterCount}</span>
          </Button>
          <div
            id="mc-filters"
            className={`${filtersOpen ? "flex" : "hidden"} min-w-0 flex-wrap items-center gap-3 sm:contents`}
          >
          <Input
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="focus-ring h-9 w-24 rounded-[var(--radius-brand)] border border-border bg-surface-2 px-3 text-sm text-text-main metric"
            />
            <input
              type="number"
              placeholder="≤"
              ...
              className="focus-ring h-9 w-16 rounded-[var(--radius-brand)] border border-border bg-surface-2 px-3 text-sm text-text-main metric"
            />
          </div>
          </div>
          {/* On phone this strip scrolls sideways instead of wrapping into three
              rows, the treatment wide control rows already get elsewhere. */}
          <div className="flex items-center gap-2 overflow-x-auto scroll-thin-x sm:ms-auto sm:overflow-visible">
            <Button size="sm" variant="ghost" icon={keysOpen ? "expand_less" : "tune"} onClick={() => setKeysOpen(!keysOpen)} className="shrink-0">
              Override keys ({coveredCount})
            </Button>
            <Button size="sm" variant={hiddenView ? "primary" : "ghost"} icon="visibility_off"
              onClick={() => setHiddenView(!hiddenView)} className="shrink-0">
              Hidden ({hidden.size})
            </Button>
            <Button size="sm" icon="add" onClick={() => setAddOpen(!addOpen)} className="shrink-0">Add</Button>
          </div>
        </div>
        ...
        {selected.size > 0 && (
          <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-brand-soft border-b border-border-subtle">
            ...
            <input
              ...
              className="focus-ring min-h-11 w-28 rounded-[var(--radius-brand)] border border-border bg-surface-2 px-3 text-sm text-text-main metric"
            />
        ...
        {addOpen && (
          <div className="flex flex-wrap items-end gap-3 px-4 py-3 border-b border-border-subtle bg-surface-2">
            <div className="flex-1 min-w-[240px]">
              ...
              <input
                list="mc-key-options"
                className="focus-ring w-full h-9 rounded-[var(--radius-brand)] border border-border bg-surface-2 px-3 text-sm text-text-main"
                  ...
                  return (
                    <div key={k} className="flex items-center justify-between gap-3 px-4 py-3 rounded-[var(--radius-brand)] border border-border-subtle bg-surface">
                      <div className="flex items-center gap-2 min-w-0 flex-wrap">
                        <code className="text-xs font-mono bg-surface-2 text-text-main px-1.5 py-1 rounded">{k}</code>
                        <span className="text-sm font-mono text-text-main metric">{fmtInt(v)}</span>
                        {hitCount === 0 && (
                          <span className="inline-flex items-center gap-1 text-xs px-1.5 py-1 rounded bg-danger-soft text-danger">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10.5px] text-text-subtle metric">{hitCount} models</span>
                        <Button
                          variant="bare" size="icon-sm"
            ...
            <thead>
              <tr className="border-b border-border-subtle text-start text-xs text-text-muted">
                <th scope="col" className="px-4 py-3 w-8">
                  {/* The label is the target. A 16px box is under the 32px floor
                      design-system.md section 6 sets, and a checkbox cannot be
                      drawn larger without drawing a larger checkbox, so the
                      activation region is widened instead. */}
                  <label className="-my-3 flex h-11 w-11 cursor-pointer items-center">
                ...
                <th scope="col" aria-sort={sort.key === "provider" ? (sort.dir === 1 ? "ascending" : "descending") : "none"} className="px-4 py-3 font-medium">
                  <button className="focus-ring hit-44 rounded-sm cursor-pointer hover:text-text-main transition-colors duration-150" onClick={() => toggleSort("provider")}>
                ...
                <th scope="col" aria-sort={sort.key === "model" ? (sort.dir === 1 ? "ascending" : "descending") : "none"} className="px-4 py-3 font-medium">
                  <button className="focus-ring hit-44 rounded-sm cursor-pointer hover:text-text-main transition-colors duration-150" onClick={() => toggleSort("model")}>
                ...
                <th scope="col" aria-sort={sort.key === "eff" ? (sort.dir === 1 ? "ascending" : "descending") : "none"} className="px-4 py-3 font-medium">
                  <button className="focus-ring hit-44 rounded-sm cursor-pointer hover:text-text-main transition-colors duration-150" onClick={() => toggleSort("eff")}>
                    Effective window{sortIcon("eff")}
                  </button>
                </th>
                <th scope="col" className="px-4 py-3 font-medium">Source</th>
                <th scope="col" className="px-4 py-3 font-medium text-end">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                visible.map((r) => (
                  <tr key={r.fullKey} className="border-b border-border-subtle last:border-b-0 hover:bg-surface-2 transition-colors duration-150">
                    ...
                    <td className="px-4 py-3 min-w-0">
                      ...
                      {!r.enabledModel && (
                        <span
                          ...
                          className="text-xs px-1.5 py-1 rounded bg-surface-2 text-text-muted ms-1"
                        >
                          Inactive
                        </span>
                      )}
                      <code className="text-[10.5px] font-mono bg-surface-2 px-1 py-1 rounded text-text-muted ms-1">{r.provider}</code>
                    </td>
                    <td className="px-4 py-3 max-w-[260px]">
                      <code className="text-xs font-mono bg-surface-2 text-text-main px-1.5 py-1 rounded truncate block">{r.model}</code>
                      {r.name && r.name !== r.model ? <span className="text-xs text-text-muted truncate block">{r.name}</span> : null}
                    </td>
                      ...
                      {editing?.rowKey === r.fullKey ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="focus-ring h-7 w-24 rounded-[var(--radius-brand)] border border-border bg-surface-2 px-2 text-xs text-text-main metric"
                          />
                          <Button
                    ...
                    <td className="px-4 py-3">
                      {r.src ? (
                        <span title={r.src.key} className="inline-flex items-center gap-1 text-xs px-1.5 py-1 rounded border border-border bg-surface-2 text-text-muted">
