  ...
  return (
    <div className="relative flex-1 flex flex-col h-full min-h-0 min-w-0 bg-bg text-text-main overflow-hidden">
      <div className="relative mx-auto flex flex-1 h-full min-h-0 w-full max-w-4xl flex-col">
        <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3 lg:px-6">
          <div ref={modelMenuRef} className="relative">
            <button
              ...
              className="flex items-center gap-3 focus-ring rounded-[var(--radius-brand)] border border-border bg-surface-2 px-4 py-3 text-left transition-colors duration-150 hover:bg-surface-3"
            >
            ...
            {modelMenuOpen ? (
              <div className="absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-[var(--radius-brand-lg)] border border-border bg-surface shadow-[var(--shadow-elev)]">
                        ...
                        {group.models.map((model) => {
                          ...
                          return (
                            <button
                              ...
                              className={`focus-ring rounded-[var(--radius-brand)] border px-3 py-3 text-left transition-colors duration-150 ${isActive ? "border-brand-line bg-brand-soft" : "border-border bg-surface-2 hover:bg-surface-3"}`}
                            >
        ...
        {historyOpen ? (
          <div ref={historyMenuRef} className="absolute right-4 top-[72px] z-20 w-[min(360px,calc(100vw-2rem))] rounded-[var(--radius-brand-lg)] border border-border bg-surface p-2 shadow-[var(--shadow-elev)] lg:right-6">
              ...
              {sessionItems.length === 0 ? (
                ...
                return (
                  <button
                    ...
                    className={`focus-ring w-full rounded-[var(--radius-brand)] border px-3 py-3 text-left transition-colors duration-150 ${isActive ? "border-brand-line bg-brand-soft" : "border-border bg-surface-2 hover:bg-surface-3"}`}
                  >
                    <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-1 flex-col min-h-0">
              ...
              {currentMessages.map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-6`}>
                    <div className={`max-w-[min(88%,42rem)] ${isUser ? "rounded-[var(--radius-brand-lg)] bg-surface-2 px-5 py-3.5 text-text-main" : "text-text-main"}`}>
                      ...
                      {message.attachments?.length ? (
                        <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 mt-2">
                          {message.attachments.map((attachment) => (
                            <a key={attachment.id} href={attachment.dataUrl} target="_blank" rel="noreferrer" className="overflow-hidden rounded-[var(--radius-brand)] border border-border bg-surface-2">
          ...
          <div className="shrink-0 pt-2">
            ...
            <div className="mx-auto w-full max-w-3xl px-4 pb-2">
              <div className="rounded-[var(--radius-brand-lg)] border border-border bg-surface-2 px-3 pt-3 pb-2 shadow-[var(--shadow-soft)]">
                <textarea
                  value={draft}
