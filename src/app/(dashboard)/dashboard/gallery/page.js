...
import { classifyConnection } from "../providers/connectionStatus";
...
// A primitive with a product consumer is documented in the states that consumer
// actually produces, not only in the tones it shipped with. The eight provider
// connection states come from `providers/connectionStatus.js` rather than from a
// list copied out of it, so a ninth state or a retoned one appears here without
// anyone remembering to update the gallery. The clock is fixed so a lock-bearing
// fixture classifies the same way on every render.
const NOW = Date.UTC(2026, 0, 1, 12, 0, 0);

const CONNECTION_FIXTURES = [
  { isActive: true, testStatus: "active" },
  { isActive: true, testStatus: "unavailable", lastErrorType: "upstream_unavailable" },
  { isActive: true, lastErrorType: "upstream_rate_limited", modelLock_gpt5: NOW + 41_000 },
  { isActive: true, lastErrorType: "network_error", modelLock_sonnet: NOW + 900_000 },
  { isActive: true, expiresAt: NOW - 86_400_000, lastErrorType: "token_expired" },
  { isActive: true, testStatus: "error", errorCode: 500 },
  { isActive: false },
  { isActive: true },
];
...
const FRESHNESS = [
  ...
  { tone: "degraded", label: "stale, 14m" },
  { tone: "idle", label: "no traffic yet" },
  { tone: "idle", label: "no data" },
];

function Section({ id, title, note, children }) {
  return (
    <section id={id} className="border-t border-border pt-5.5">
      <div className="mb-4 flex items-baseline justify-between gap-4">
        <h3 className="font-mono text-[10px] uppercase tracking-[0.14em] text-text-muted">
          {title}
        </h3>
        {note ? (
          <p className="max-w-prose text-end text-xs text-text-subtle">{note}</p>
        ) : null}
      </div>
...
export default function GalleryPage() {
  return (
    <div className="mx-auto max-w-[1100px] space-y-8 px-5.5 py-8">
      ...
      <Section id="status" title="Status token" note="Colour is paired with a glyph and a word, so status survives colour blindness and a monochrome screenshot.">
        ...
        <Bay label="Connection state, all eight">
          {CONNECTION_FIXTURES.map((conn) => {
            const s = classifyConnection(conn, NOW);
            return (
        ...
        <Bay label="Masthead">
          {FRESHNESS.map((f) => (
            <StatusToken key={f.label} tone={f.tone}>
              {f.label}
            </StatusToken>
          ))}
          <StatusToken tone="degraded">3 upstreams degraded</StatusToken>
        </Bay>
      </Section>
      ...
      <Section
        id="readout"
        title="Readout"
        note="Large type is reserved for a number that answers a question. Quantities are tabular. A measurement, a value that could not be computed, and a value nothing ever recorded are three states, and a dash for all three tells an operator nothing."
      >
        ...
        <Bay label="Measured, not computable, never recorded">
          <div className="flex flex-wrap gap-8 border border-border bg-surface px-4 py-3">
            <Readout label="Throughput" value="0.31" unit="req/min" />
            <div>
              <Readout label="Error rate" value={NOT_COMPUTABLE} size="sm" />
              <p
                className="mt-1 line-clamp-2 text-xs leading-snug text-text-muted"
                title="no requests in the window: src/app/api/system/state/route.js"
              >
                no requests in the window
              </p>
            </div>
            <div>
              <Readout label="Failovers" value={NOT_RECORDED} size="sm" />
              <p
                className="mt-1 line-clamp-2 text-xs leading-snug text-text-muted"
                title="no column records a failover: src/lib/usageDb.js"
              >
                no column records a failover
              </p>
            </div>
          </div>
        </Bay>
      ...
      <Section id="table" title="Table" note="Structure, not card chrome. Numeric columns are tabular and right aligned.">
        <div className="w-full border border-border bg-surface px-4 py-3">
          <DataTable label="Upstream channels by request volume">
            <THead>
              <TR>
                <TH>Upstream</TH>
                <TH>Channel</TH>
      ...
      <Section id="channels" title="Channel list" note="A combo's fallback order drawn as the sequence it is. The number is the channel's stable address.">
        <div className="w-full max-w-[560px]">
          <FlowStrip junction="Fallback — try in order" channels={3} className="mb-1.5" />
          <ChannelList>
            <Channel
              index={1}
              title="openai-codex"
      ...
      <Section id="states" title="Empty, error and loading">
        <div className="w-[320px]">
          <EmptyState
            ...
            description="Add one to route traffic through an upstream TokenProxy does not ship with."
