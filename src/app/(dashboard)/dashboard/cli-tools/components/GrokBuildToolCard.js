  ...
  return (
    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
      <div className="sm:text-end">
        ...
        {help && <p className="mt-1 text-xs leading-tight text-text-muted">{help}</p>}
      </div>
      ...
      <div className="relative w-full min-w-0">
        ...
        {value && (
          <Button
            ...
            type="button"
            onClick={() => onChange("")}
            aria-label="Clear (inherit main model for subagents)" className="absolute end-1 top-1/2 -translate-y-1/2 text-text-muted hover:text-danger"
    ...
    <Card padding="sm" className="overflow-hidden">
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          {checking && <div className="flex items-center gap-2 text-text-muted"><span aria-hidden="true" className="material-symbols-outlined animate-spin">progress_activity</span><span>Checking Grok Build...</span></div>}
          ...
          {!checking && grokStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {tool.notes?.length > 0 && (
                  <div className="mb-2 flex flex-col gap-2">
                    {tool.notes.map((note, index) => (
                      <div key={index} className={`flex items-start gap-2 rounded p-2 text-xs ${note.type === "warning" ? "bg-warning-soft text-warning" : "bg-info-soft text-info"}`}>
                        <span aria-hidden="true" className="material-symbols-outlined mt-1 text-[14px]">{note.type === "warning" ? "warning" : "info"}</span>
                        <span>{note.text}</span>
                      </div>
                    ))}
                  </div>
                ...
                {configuredModel?.base_url && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    ...
                    <span aria-hidden="true" className="material-symbols-outlined dir-icon hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                    <span className="min-w-0 break-all rounded bg-surface-2 px-2 py-2 text-xs text-text-muted metric sm:py-1.5">{configuredModel.base_url} · {configuredModel.model}{configuredModel.context_window ? ` · ${(configuredModel.context_window / 1000).toLocaleString()}K ctx` : ""}</span>
                  </div>
                )}
                ...
                <div className="my-1 border-t border-border pt-3">
                  <div className="mb-2 flex items-start gap-2">
                    <span aria-hidden="true" className="material-symbols-outlined text-brand text-[16px]">account_tree</span>
                    <div>
              ...
              {message && <div className={`flex items-center gap-2 px-2 py-1.5 rounded text-xs ${message.type === "success" ? "bg-success-soft text-success border border-success-line" : "bg-danger-soft text-danger border border-danger-line"}`}><span aria-hidden="true" className="material-symbols-outlined text-[14px]">{message.type === "success" ? "check_circle" : "error"}</span><span>{message.text}</span></div>}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button variant="secondary" size="sm" icon="restore" onClick={handleReset} disabled={!grokStatus?.hasTokenProxy} loading={restoring} className="w-full sm:w-auto">Reset</Button>
