...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasTokenProxy) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["tokenproxy"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["tokenproxy"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.tokenproxy]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_TOKENPROXY_API_KEY"
env_file = "provider-tokenproxy.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.tokenproxy.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_TOKENPROXY_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-tokenproxy.env",
        content: envContent,
      },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-warning-soft border border-warning-line rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if tokenproxy is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note, idx) => (
                      <div key={idx} className={`flex items-start gap-2 p-2 rounded text-xs ${
                        note.type === "info" ? "bg-info-soft text-info" :
                        note.type === "warning" ? "bg-warning-soft text-warning" :
                        "bg-surface-2 text-text-muted"
                      }`}>
                        <span aria-hidden="true" className="material-symbols-outlined text-[14px] mt-1">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["tokenproxy"]?.base_url && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <input type="text" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="cc/claude-opus-4-7" className="w-full min-w-0 ps-2 pe-8 py-2 bg-surface rounded border border-border text-xs focus-ring sm:py-1.5" />
                ...
                {/* Usage hint */}
                <div className="flex flex-col gap-1 p-3 bg-info-soft border border-info-line rounded-lg">
                  <p className="text-xs font-medium text-info">Usage:</p>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile tokenproxy</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile tokenproxy --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button variant="primary" size="sm" icon="save" onClick={handleApplySettings} disabled={!selectedModel} loading={applying}>Apply</Button>
                <Button variant="secondary" size="sm" icon="restore" onClick={handleResetSettings} disabled={!jcodeStatus?.hasTokenProxy} loading={restoring}>Reset</Button>
