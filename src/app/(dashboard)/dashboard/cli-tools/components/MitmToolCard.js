    ...
    <>
      <Card padding="sm" className="overflow-hidden">
        <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
          ...
          <span aria-hidden="true" className={`material-symbols-outlined text-text-muted text-[20px] transition-transform ${isExpanded ? "rotate-180" : ""}`}>
            expand_more
          </span>
        </div>
          ...
          <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
            ...
            {mitmHosts.length > 0 && (
              <div className="mt-2 rounded-md border border-border bg-surface-2 px-2 py-1.5">
                ...
                <ul className="list-none space-y-1 font-mono text-xs text-text-muted break-all">
                  {mitmHosts.map((h) => (
            ...
            {/* Info */}
            <div className="flex flex-col gap-1 text-xs text-text-muted px-1">
              <p>Toggle DNS to redirect {tool.name} traffic through TokenProxy via MITM.</p>
              {!dnsActive && (
              ...
              <div className="flex flex-col gap-2">
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    <span className="text-xs font-semibold text-text-main sm:text-end">{model.name}</span>
                    ...
                    <div className="relative w-full min-w-0">
                      <input
                        ...
                        disabled={!dnsActive}
                        className={`w-full min-w-0 ps-2 pe-8 py-2 bg-surface rounded border border-border text-xs focus-ring sm:py-1.5 ${!dnsActive ? "opacity-50 cursor-not-allowed" : ""}`}
                      />
