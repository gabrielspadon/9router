...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["tokenproxy"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["tokenproxy"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setSelectedModel(primaryModel.replace("tokenproxy/", ""));
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `tokenproxy/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
              ...
              <div className="flex flex-col gap-3 p-4 bg-warning-soft border border-warning-line rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-warning">Open Claw CLI not detected locally</p>
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["tokenproxy"]?.baseUrl && (
                ...
                {(openclawStatus.agents || []).filter(a => a.agentDir).map((agent) => (
                  <div key={agent.id} className="flex items-center gap-2 ps-4">
                    <span className="w-32 shrink-0 text-xs text-brand break-words sm:text-end" title={agent.name || agent.id}>Agent {agent.name || agent.id}</span>
                    <span aria-hidden="true" className="material-symbols-outlined dir-icon hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                    <div className="relative w-full min-w-0">
                      <input
                        type="text"
                      ...
                      />
                      {agentModels[agent.id] && <Button variant="bare" size="icon-sm" onClick={() => setAgentModels(prev => ({ ...prev, [agent.id]: "" }))} aria-label="Clear" className="absolute end-1 top-1/2 -translate-y-1/2 text-text-muted hover:text-danger" title="Clear"><span aria-hidden="true" className="material-symbols-outlined text-[14px]">close</span></Button>}
                    </div>
                    <Button
                      variant="secondary"
                      size="sm"
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="secondary" size="sm" icon="restore" onClick={handleResetSettings} disabled={!openclawStatus?.hasTokenProxy} loading={restoring}>Reset</Button>
