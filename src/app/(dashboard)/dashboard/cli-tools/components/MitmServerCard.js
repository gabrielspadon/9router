    ...
    <>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <span aria-hidden="true" className="material-symbols-outlined text-brand text-[20px]">security</span>
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs text-text-muted" data-i18n-skip="true">
              {[
                ...
                <span key={label} className={`flex items-center gap-1 px-1.5 py-1 rounded ${ok ? "text-success" : "text-text-muted"}`}>
          ...
          <div className="px-2 py-2 rounded-lg bg-surface-2 border border-border flex flex-col gap-2">
            <p className="text-xs text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from TokenProxy
            </p>
            <p className="text-xs text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → TokenProxy → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-end sm:text-sm">TokenProxy Base URL</span>
              ...
              <input
                ...
                disabled={isRunning}
                className="flex-1 min-h-11 min-w-0 px-2 py-1.5 bg-surface rounded border border-border text-xs text-text-main focus-ring disabled:opacity-50"
              />
            </div>
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_tokenproxy (default)"}
                  className="flex-1 min-h-11 min-w-0 px-2 py-1.5 bg-surface rounded border border-border text-xs text-text-main focus-ring"
                />
                {apiKeys?.length > 0 && (
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center" data-i18n-skip="true">
            ...
            {isRunning ? (
              ...
              <Button
                ...
                title={serverIsWindows && !isAdmin ? "Windows will request administrator permission" : undefined}
                className="w-full sm:w-auto"
              >
                Start Server
              </Button>
          ...
          {/* Action error */}
          {actionError && (
            <div className="flex items-start gap-2 px-2 py-1.5 rounded text-xs bg-danger-soft text-danger border border-danger-line">
              <span aria-hidden="true" className="material-symbols-outlined text-[14px] mt-1 shrink-0">error</span>
              <span>{actionError}</span>
            </div>
          )}

          {/* Windows elevation notice */}
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-warning-soft text-warning border border-warning-line">
              ...
              <span>Windows will ask for administrator permission when you start MITM</span>
            </div>
          )}
        </div>
      </Card>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 flex w-full max-w-sm flex-col gap-4 rounded-xl border border-border bg-surface p-5.5 shadow-[var(--shadow-elev)] sm:p-5.5">
      ...
      {port443Conflict && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 flex w-full max-w-md flex-col gap-4 rounded-xl border border-border bg-surface p-5.5 shadow-[var(--shadow-elev)] sm:p-5.5">
