...
export function replaceGuideVariables(text, {
  ...
  const keyToUse = (apiKey && apiKey.trim())
    ? apiKey
    : (!cloudEnabled ? "sk_tokenproxy" : "your-api-key");
...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = [], cloudEnabled = false, tunnelEnabled = false }) {
        ...
        {tool.notes.map((note, index) => {
          ...
          return (
            <div key={index} className={`flex items-start gap-3 p-3 rounded-lg border ${bgClass}`}>
              <span aria-hidden="true" className={`material-symbols-outlined text-[20px] ${iconClass}`}>{icon}</span>
        ...
        {canShowGuide() && tool.guideSteps.map((item) => (
          <div key={item.step} className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.desc && <p className="text-sm text-text-muted mt-1">{item.desc}</p>}
        ...
        {canShowGuide() && tool.codeBlock && (
          <div className="mt-2">
            <div className="flex items-center justify-between mb-2">
              <span className="font-mono text-xs text-text-muted">{tool.codeBlock.language}</span>
              <Button
                variant="secondary"
                size="sm"
  ...
  const renderIcon = () => {
    ...
    if (tool.icon) {
      return <span aria-hidden="true" className="material-symbols-outlined text-xl" style={{ color: tool.color }}>{tool.icon}</span>;
    }
  ...
  return (
    <Card padding="sm" className="overflow-hidden overflow-x-hidden">
      ...
      {isExpanded && (
        <div className="mt-5.5 pt-5.5 border-t border-border">
          {renderGuideSteps()}
        </div>
      )}
