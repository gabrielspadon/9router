  ...
  return (
    <>
      <button
        ...
        className="focus-ring hit-44 relative flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3 py-1.5 text-xs font-medium text-text-muted hover:text-text-main hover:border-brand-line transition-colors duration-150"
      ...
      >
        ...
        {unseenCount > 0 && (
          <span className="metric absolute -top-1.5 -end-1.5 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-brand-solid text-brand-on text-xs font-semibold px-1">
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="New Models Discovery">
      <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-5.5 text-text-muted text-sm">
          ...
          <div className="flex items-center justify-center gap-2 py-5.5 text-danger text-sm">
              ...
              <div key={group.providerAlias} className="border border-border rounded-lg overflow-hidden">
                <div className="flex items-center justify-between px-3 py-2 bg-surface-2 border-b border-border">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="metric text-xs text-text-muted bg-bg rounded-full px-1.5 py-1">
                      {group.models.length}
                    </span>
                  </div>
                </div>
                <div className="divide-y divide-border">
                  {group.models.map((m) => (
                    <div key={m.modelId} className="flex items-center gap-2 px-3 py-1.5">
                      {m.isNew && (
                        <span className="shrink-0 min-w-[32px] text-center text-xs font-semibold text-info-on bg-info-solid rounded px-1 py-1">
                          NEW
                        </span>
                      )}
                      {m.isFree && (
                        <span className="shrink-0 min-w-[32px] text-center text-xs font-semibold text-text-muted bg-surface-2 border border-border rounded px-1 py-1">
                          FREE
                        </span>
                      )}
