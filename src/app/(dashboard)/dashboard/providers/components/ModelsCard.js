...
export function ModelRow({ model, fullModel, copied, onCopy, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting }) {
  const borderColor = testStatus === "ok" ? "border-success-line" : testStatus === "error" ? "border-danger-line" : "border-border";
  // Same status, same source of truth as the border above; a literal green does
  // not follow the theme the way `border-success-line` does.
  const iconColor = testStatus === "ok" ? "var(--color-success)" : testStatus === "error" ? "var(--color-danger)" : undefined;

  return (
    <div className={`group px-3 py-2 rounded-lg border ${borderColor} hover:bg-surface-2`}>
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-1">
          ...
          {model.name && <span className="text-xs text-text-subtle italic ps-1">{model.name}</span>}
        </div>
        {onTest && (
        ...
        {isFree && <span className="text-xs font-semibold text-text-muted bg-surface-2 border border-border px-1.5 py-1 rounded">FREE</span>}
        {isCustom && (
          <Button variant="bare" size="icon-sm" onClick={onDeleteAlias} className="hover:bg-danger-soft text-text-muted hover:text-danger opacity-0 group-hover:opacity-100 transition-opacity ms-auto" title="Remove custom model">
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="focus-ring flex items-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-border text-xs text-text-muted hover:text-brand hover:border-brand-line transition-colors duration-150"
          >
