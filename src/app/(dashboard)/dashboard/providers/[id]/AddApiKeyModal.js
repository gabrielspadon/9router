...
export default function AddApiKeyModal({
  ...
  // Credentials a provider needs beside its API key (Google PSE's search engine
  // id). Declared on the registry entry so the form stays data-driven.
  const extraFields = AI_PROVIDERS?.[provider]?.extraFields || null;
  ...
  const handleSubmit = async () => {
    if (!provider) return;
    // A custom compatible endpoint may take no credential; requiring one here
    // forced a dummy key that was then sent upstream (#1523).
    if (!isOllamaLocal && !isCompatible && !formData.apiKey) return;
    if (!isOllamaLocal) {
        ...
        {mode === "bulk" && (
          <div className="flex flex-col gap-3">
            <p className="text-xs text-text-muted">
              {isCloudflareAi ? (
                <>
                  One key per line. Format: <code>name|apiKey|accountId</code>{" "}
                ...
                <>
                  ...
                  <code>apiKey</code> (auto-named by index).
                </>
              )}
            </p>
            <textarea
          ...
          <>
            ...
            {extraFields?.map((field) => (
              <Input
                key={field.key}
                label={field.label}
                value={formData.extraValues?.[field.key] || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    extraValues: { ...formData.extraValues, [field.key]: e.target.value },
                  })
                }
            ...
            {!isOllamaLocal && (
              <div className="flex gap-2">
                <Input
                  label={isCompatible ? `${credentialLabel} (optional)` : credentialLabel}
                  type={isCookie ? "text" : "password"}
                  value={formData.apiKey}
                  onChange={(e) =>
                    setFormData({ ...formData, apiKey: e.target.value })
                  }
                  placeholder={isCompatible ? "Leave empty for an endpoint that needs no key" : credentialPlaceholder}
                  className="flex-1"
                />
                <div className="pt-5.5">
                  <Button
            ...
            {isXaiApiKey && (
              <p className="text-xs text-text-muted">
                Use a direct xAI API key from console.x.ai. This is separate
                from Grok Build OAuth.
              </p>
            )}
            {isCookie && authHint && (
              <p className="text-xs text-text-muted">
            ...
            {isCompatible && (
              <Input
                ...
                placeholder={
                  isAnthropic ? "claude-3-5-sonnet-latest" : "gpt-4o-mini"
                }
              />
            )}
            {isOllamaLocal && (
              <p className="text-xs text-text-muted">
                Leave blank to use <code>http://localhost:11434</code>. For
                remote Ollama, enter the full host URL (e.g.{" "}
            ...
            {isCompatible && (
              <p className="text-xs text-text-muted">
                Enter the model ID exactly as your compatible endpoint expects
                it. This model will be saved as the connection default.
              </p>
            )}
            {isCloudflareAi && (
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={
                  saving ||
                  (!isOllamaLocal && !formData.name) ||
                  (!isOllamaLocal && !isCompatible && !formData.apiKey) ||
