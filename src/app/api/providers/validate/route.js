...
async function probeHuggingFaceToken(apiKey) {
  try {
    const res = await fetchWithTimeout(HUGGINGFACE_WHOAMI_URL, {
...
async function probeGitlawbOpenGatewayToken(apiKey) {
  ...
  try {
    const res = await fetchWithTimeout(creditsUrl, {
      method: "GET",
      headers: { Authorization: `Bearer ${apiKey}` },
      signal: AbortSignal.timeout(8000),
    });
...
async function probeWebProvider(provider, apiKey) {
  ...
  // Probe the provider's no-charge endpoint where it declares one: baseUrl is
  // the live search endpoint, so validating a key there spends the user's credit.
...
async function probeMediaProvider(provider, apiKey) {
  ...
  switch (cfg.authHeader) {
    ...
    // No auth scheme declared for this media config, so no probe can be built.
    // Returning null here handed the provider to the switch below, whose default
    // branch only knows OpenAI-compatible chat providers and answers 400
    // "Provider validation not supported" — which the modal renders as "Invalid"
    // for every key that is typed. fal-ai, stability-ai, recraft, runwayml,
    // comfyui and sdwebui all land here (#993). Accept best-effort, exactly as
    // the no-config case above does, and let the connection Test do the real
    // probe; a media config that DOES name a scheme is still probed for real.
    default: return true;
  }
  ...
  try {
    ...
    try {
      if (isOpenAICompatibleProvider(provider)) {
        ...
        // Only an auth status condemns the key from the listing alone. Plenty of
        // OpenAI-compatible gateways serve no /models and answer 404 or 405, and
        // treating that as a bad key reported every hand-added endpoint as
        // invalid (#994). Probe the endpoint the connection will actually use
        // before deciding — the same shape the custom-embedding branch below
        // already uses. A 404 from the chat probe still fails, because there it
        // means the base URL or the model is wrong rather than the key (#2032).
        ...
        const chatRes = await fetchWithTimeout(`${baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({
            model: node.defaultModel || "test",
            messages: [{ role: "user", content: "ping" }],
            max_tokens: 1,
          }),
        });
        isValid = ![401, 403, 404].includes(chatRes.status);
      ...
      if (provider === "huggingface") {
        return NextResponse.json(await probeHuggingFaceToken(apiKey));
      }

      if (provider === "gitlawb-opengateway") {
        return NextResponse.json(await probeGitlawbOpenGatewayToken(apiKey));
      }
