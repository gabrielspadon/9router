...
const hasTokenProxyConfig = (config) => {
  ...
  return !!config.provider["tokenproxy"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["tokenproxy"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("tokenproxy/") ? config.model.replace(/^tokenproxy\//, "") : null,
...
// POST - Apply TokenProxy as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    // Preserve any existing tokenproxy provider entry and its models
    const existingProvider = config.provider["tokenproxy"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["tokenproxy"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `tokenproxy/${finalActive}`;
      }
    }
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("tokenproxy/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove TokenProxy provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["tokenproxy"]?.models) {
      delete config.provider["tokenproxy"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["tokenproxy"].models).length === 0) {
        delete config.provider["tokenproxy"];
        if (config.model?.startsWith("tokenproxy/")) delete config.model;
      } else if (config.model === `tokenproxy/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["tokenproxy"].models);
        config.model = `tokenproxy/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire tokenproxy provider
      if (config.provider) delete config.provider["tokenproxy"];
      if (config.model?.startsWith("tokenproxy/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("tokenproxy/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "TokenProxy settings removed from OpenCode",
    });
  } catch (error) {
