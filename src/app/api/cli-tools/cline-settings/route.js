...
const hasTokenProxyConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("tokenproxy"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasTokenProxy: hasTokenProxyConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "TokenProxy settings removed from Cline" });
  } catch (error) {
