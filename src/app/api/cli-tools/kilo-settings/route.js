...
const hasTokenProxyConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["tokenproxy"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("tokenproxy");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasTokenProxy: hasTokenProxyConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "TokenProxy", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "TokenProxy settings removed from Kilo Code" });
  } catch (error) {
