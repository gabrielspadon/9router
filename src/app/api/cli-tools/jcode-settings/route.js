...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-tokenproxy.env");
};
...
const hasTokenProxyConfig = (config) => {
  ...
  if (providers["tokenproxy"]) return true;
...
export async function GET() {
  ...
  const hasTokenProxy = hasTokenProxyConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_TOKENPROXY_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile tokenproxy",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["tokenproxy"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_TOKENPROXY_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "tokenproxy configuration removed from jcode",
    });
  } catch (error) {
