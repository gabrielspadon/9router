...
// Check if settings has TokenProxy customModels
const hasTokenProxyConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:TokenProxy"));
};
...
// POST - Update TokenProxy customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing TokenProxy configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:TokenProxy"));
...
// DELETE - Remove TokenProxy customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove TokenProxy customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:TokenProxy"));
