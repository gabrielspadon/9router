...
// Check if settings has TokenProxy config
const hasTokenProxyConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["tokenproxy"];
};

// Read per-agent models.json and return current model id (without "tokenproxy/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["tokenproxy"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasTokenProxy: hasTokenProxyConfig(settings),
...
// POST - Update TokenProxy settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `tokenproxy/${model}`;

    // Remove all old tokenproxy/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("tokenproxy/"))
    ...
    // Add fresh tokenproxy models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`tokenproxy/${m}`] = {};
    });

    // Remove old tokenproxy model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("tokenproxy/")) {
    ...
    // Update models.providers.tokenproxy with all models
    settings.models.providers["tokenproxy"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `tokenproxy/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove TokenProxy from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["tokenproxy"];
    ...
    // Remove tokenproxy models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("tokenproxy/"));
    ...
    // Reset agents.defaults.model.primary if it uses tokenproxy
    if (settings.agents?.defaults?.model?.primary?.startsWith("tokenproxy/")) {
