...
// Free/freeTier registry entries exposing a public catalog we can filter, plus
// an "oauth" provider whose modelsFetcher.type is itself a "*-free" filter
// contract (openrouter-free, opencode-free, mimo-free, nous-free). That last
// clause is #1995: kilocode is category "oauth" (chat calls need a real
// device-code token) but its modelsFetcher hits a public, unauthenticated
// catalog endpoint (verified: api.kilo.ai/api/gateway/models returns 200 with
// no Authorization header) already filtered down to zero-priced models by the
// "openrouter-free" contract, so syncing it into customModels is as safe as
// syncing openrouter's. The "*-free" suffix check keeps this from also
// pulling in an apikey/oauth provider whose fetcher lists its FULL paid
// catalog (kenari, chutes, groq, meta, perplexity-agent, vercel-ai-gateway
// all declare "openai"/"openai-list", neither of which narrows to free).
export function getSyncTargets() {
  return REGISTRY
    .filter((r) =>
      (r.category === "free" || r.category === "freeTier" ||
        (r.category === "oauth" && !!r.modelsFetcher?.type?.endsWith("-free"))) &&
      r.modelsFetcher?.url &&
