...
function ModelItem({ index, model, isFirst, isLast, onEdit, onMoveUp, onMoveDown, onRemove }) {
  ...
  return (
    <div className="group flex min-w-0 items-center gap-1.5 rounded-md bg-surface-2 px-2 py-1 transition-colors duration-150 hover:bg-surface-3">
      ...
      {editing ? (
        <input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={commit} onKeyDown={handleKeyDown}
          className="focus-ring min-w-0 flex-1 rounded border border-brand-solid bg-surface px-1.5 py-1 font-mono text-xs text-text-main" />
      ) : (
        <div className="min-w-0 flex-1 cursor-text truncate rounded px-1.5 py-1 font-mono text-xs text-text-main hover:bg-surface-2"
          ...
          <div>
            {forcePrefix ? (
              <>
                ...
                {/* rounded-l / rounded-r / border-r-0 stay physical: this joins a
                    prefix chip to a font-mono model-id input, which reads LTR. */}
