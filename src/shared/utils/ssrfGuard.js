...
// undici reports a connector or DNS refusal as TypeError("fetch failed") with the
// real cause attached, so the guard's marker has to be found down the cause chain.
export function findBlockedError(err) {
  for (let current = err, depth = 0; current && depth < 5; current = current.cause, depth += 1) {
    if (current.code === SSRF_BLOCKED_ERROR_CODE) return current;
  }
  return null;
}
