...
const previousDefaultRegExp = /^# tokenproxy-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# tokenproxy-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via TokenProxy gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# tokenproxy-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# tokenproxy-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
