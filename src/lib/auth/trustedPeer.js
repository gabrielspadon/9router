// x-tp-real-ip is only trustworthy when custom-server.js stamped it from the TCP socket.
...
export function hasTrustedPeerHeaders(request) {
  const token = process.env.TOKENPROXY_PEER_TOKEN;
  return Boolean(token) && request.headers.get("x-tp-peer-token") === token;
}
