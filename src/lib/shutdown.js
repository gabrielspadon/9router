const state = (globalThis.__tokenproxyShutdownState ??= {
  flushers: new Set(),
  priorities: new Map(),
  ...
  state.shutdownPromise = (async () => {
    ...
    for (const priority of [...groups.keys()].sort((left, right) => left - right)) {
      const results = await Promise.allSettled(
        groups.get(priority).map((flush) => Promise.resolve().then(flush))
      );
...
function installShutdownHandlers() {
  ...
  if (!process.listeners('SIGINT').includes(state.signalHandler))
    process.once('SIGINT', state.signalHandler);
  if (!process.listeners('SIGTERM').includes(state.signalHandler))
    process.once('SIGTERM', state.signalHandler);
