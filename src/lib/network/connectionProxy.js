...
const CONNECTION_PROXY_PROTOCOLS = new Set([
...
function isSupportedConnectionProxyUrl(value) {
  try {
    return CONNECTION_PROXY_PROTOCOLS.has(new URL(value).protocol);
  } catch {
    return false;
  }
}
...
export async function resolveConnectionProxyConfig(data = {}, { persistPoolSnapshot } = {}) {
  ...
  if (proxyPoolIdRaw) {
    ...
    const activePool = proxyPool && proxyPool.isActive === true && isSupportedConnectionProxyUrl(proxyUrl)
      ? { ...proxyPool, proxyUrl }
      : null;
    if (!activePool) {
      return requiredUnavailable("selected-pool-unavailable", proxyPoolIdRaw, storedStrict === true);
