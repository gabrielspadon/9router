...
export function applyOutboundProxyEnv(
  ...
  if (!enabled) {
    if (process.env.TOKENPROXY_PROXY_MANAGED === "1") {
  ...
  const wasManaged = process.env.TOKENPROXY_PROXY_MANAGED === "1";
  let managed = false;
  ...
  if (proxyUrl) {
    ...
    if (validated) {
      ...
      process.env.TOKENPROXY_PROXY_URL = validated;
      managed = true;
    }
  }
  ...
  if (noProxy) {
    ...
    process.env.TOKENPROXY_NO_PROXY = noProxy;
    managed = true;
  }

  if (managed) {
    process.env.TOKENPROXY_PROXY_MANAGED = "1";
  } else if (wasManaged) {
