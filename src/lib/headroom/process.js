...
async function awaitPidDeath(pid, { termGraceMs = 2000, killWaitMs = 1000, pollMs = 50 } = {}) {
  try {
    ...
    return isPidAlive(pid) ? false : true;
  }
...
export async function startHeadroomProxy({
  ...
  const args = [
    ...
    '--port',
    String(safePort),
    ...extrasProxyArgs({ codeAware, kompress, lossless }),
  ];
  let child;
  try {
  ...
  const pending = await new Promise((resolve, reject) => {
    ...
    const failEarlyExit = (code) => {
      ...
      e.code = 'EARLY_EXIT';
      if (settleOnce(resolve, reject, e)) {
        closeFdOnce();
        clearPid(spawnedPid);
      }
    };
  ...
  child.once('exit', () => {
    try {
      if (!isPidAlive(spawnedPid)) clearPid(spawnedPid);
    } catch {}
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) => {
      ...
      if (code === 0) {
        ...
        // The reason is already in the log, and leaving it there is how a
        // PEP 668 externally-managed-environment refusal reached the user as a
        // bare exit code with nothing to act on (#3566). Carry the tail.
        let tail = '';
        try {
          const text = fs.readFileSync(INSTALL_LOG_FILE, 'utf8').trimEnd();
          const lines = text.split(/\r?\n/).filter((l) => l.trim());
          tail = lines.slice(-6).join(' | ').slice(0, 600);
        } catch {
          // No log to read: the exit code on its own is still the truth.
        }
        const err = new Error(
