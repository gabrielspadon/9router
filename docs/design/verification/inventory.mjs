...
//
//   node .unlazy/r2/inventory.mjs before http://127.0.0.1:20135
//   node .unlazy/r2/inventory.mjs after  http://127.0.0.1:20135
...
mkdirSync(".unlazy/r2/evidence", { recursive: true });
const total = Object.values(routes).reduce((n, r) => n + (r.controls?.length || 0), 0);
writeFileSync(`.unlazy/r2/evidence/inventory-${mode}.json`,
  JSON.stringify({ mode, base: BASE, capturedRoutes: Object.keys(routes).length, total, routes, pageErrors: errors }, null, 2));
console.log(`\n${mode}: ${total} controls across ${Object.keys(routes).length} routes -> .unlazy/r2/evidence/inventory-${mode}.json`);
await browser.close();
