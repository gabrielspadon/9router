...
for (const mode of modes) {
  const jsonPath = `docs/design/evidence/raw/${mode}.json`;
  const dir = `docs/design/evidence/routes/${mode}`;
  ...
  for (const r of routes) {
    for (const theme of ["light", "dark"]) {
      for (const tag of TAGS) {
        ...
        if (!shots.includes(`${r}--${theme}--${tag}.webp`))
          problems.push(`${mode}: missing ${r}--${theme}--${tag}.webp`);
      }
    }
  }
