...
platform are in [oauth.md](oauth.md).
...
OpenClaw also only reaches a 9Router on the same machine.

## Costs in the dashboard look enormous

The cost figure is an estimate of what the same traffic would have cost against
paid APIs directly. 9Router never charges anything and has no billing system.
...
Confirm the request is not carrying `X-9Router-Token-Saver: off`, which disables
...
See [token-saver.md](token-saver.md) for the full behaviour.
...
- [getting-started.md](getting-started.md) for install and client wiring.
- [deployment.md](deployment.md) for the full environment contract.
- [ARCHITECTURE.md](ARCHITECTURE.md) for the request lifecycle and failure
  modes.
